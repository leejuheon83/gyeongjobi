  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_2fr_auto] sm:items-end">
        ...
        />
        <Button onClick={onSubmit} disabled={pending || !code.trim() || !name.trim()}>
          {pending ? "추가 중..." : "부서 추가"}
        </Button>
      </div>
...
export function DepartmentRow({
  ...
  function onRename() {
    setError(null);
    startTransition(async () => {
      const result = await renameDepartment({ id, name: draftName });
      if (result.error) {
        setError(result.error);
      } else {
        setEditing(false);
  ...
  function onToggleActive() {
    setError(null);
    startTransition(async () => {
      const result = await setDepartmentActive({ id, isActive: !isActive });
